...
namespace DataETLViaHttp.BackgroundService {
    ...
    public sealed class CronTimer : IDisposable
    {
        ...
        public CronTimer(string expression, TimeZoneInfo zone,CronFormat format)
            : this(CronExpression.Parse(expression, format), zone)
        {
        }
        ...
        public CronTimer(string expression, CronFormat foramt)
            : this(expression, TimeZoneInfo.Utc, foramt)
        {
            _format = foramt;
        }
...
#pragma warning restore IDE0052 // Remove unread private members
            ...
            public ValueTask<bool> WaitForNextTickAsync(CronTimer owner, CancellationToken cancellationToken)
            {
                lock (this)
                {
                    ...
                    _owner = owner;
                    _activeWait = true;
                    _ctr = cancellationToken.UnsafeRegister( (state) => ((State)state!).Signal(cancellationToken: cancellationToken), this);
